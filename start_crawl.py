...
for p in range(1, n_url_chunks+1):
    ...
    subprocess.run("curl http://localhost:6800/schedule.json -d project=prototyp -d spider=protospider -d URLs={} -d limit={}"
                   .format(url_chunk, config.get('spider-settings', 'limit')))
