...
import scrapy
import tldextract
from prototyp.items import Collector
...
class ProtospiderSpider(scrapy.Spider):
    name = 'protospider'
...
##################################################################
    ...
    def __init__(self, URLs="", limit=5, *args, **kwargs):
        super(ProtospiderSpider, self).__init__(*args, **kwargs)
        self.allowed_domains, self.start_urls, self.IDs = ImportURLs(URLs)
...
##################################################################    
    ...
    filetypes = set(filetype for filetype in ['mng', 'pct', 'bmp', 'gif', 'jpg', 'jpeg', 'png', 'pst', 'psp', 'tif', 'tiff', 'ai', 'drw', 'dxf', 'eps', 'ps', 'svg',
                ...
                'css?download=true', 'pdf?download=true', 'doc?download=true', 'exe?download=true', 'bin?download=true', 'rss?download=true', 'zip?download=true', 'rar?download=true'])
    ...
    #function which checks if there has been a redirect from the mup_url
    ...
    #function which reorders the urlstack, giving highest priority to "de" urls and short urls
...
##################################################################     
    ...
    def errorback(self, failure):
        ...
        if failure.check(HttpError):
            ...
            loader.add_value("mup_url", response.request.meta.get('download_slot'))
            loader.add_value("start_url", "")
            ...
            loader.add_value("crefo", response.request.meta["ID"])
            yield loader.load_item()
        elif failure.check(DNSLookupError):
            request = failure.request
            loader.add_value("mup_url", request.meta.get('download_slot'))
            loader.add_value("start_url", "")
            loader.add_value("scraped_urls", "")
            loader.add_value("redirect", [None])
            loader.add_value("scraped_text", "")
            loader.add_value("error", "DNS")
            loader.add_value("crefo", request.meta["ID"])
            yield loader.load_item() 
...
##################################################################           
    ...
    def parse(self, response):
        ...
        loader.add_value("start_url", response.url)
        ...
        print("initial parse done, going to urlstack ", response.request.meta.get('download_slot'))
    ...
    def processURLstack(self, response):
        ...
        while len(urlstack) > 0:
            ...
            #(potential bottleneck: Request has to be sent to generate fingerprint from)
                ...
                print("break ", response.request.meta.get('download_slot'))
                break
        ...
        if len(urlstack) > 0:
            ...
            print("yielding item ", response.request.meta.get('download_slot'))
            yield loader.load_item()
